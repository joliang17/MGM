import argparse
import os
os.environ['TRANSFORMERS_CACHE'] = '/fs/nexus-projects/wilddiffusion/cache/'
os.environ['HF_DATASETS_CACHE'] = '/fs/nexus-projects/wilddiffusion/cache/'
os.environ['HF_HOME'] = '/fs/nexus-projects/wilddiffusion/cache/'
import warnings
            ...
            for j in range(1):
                ...
                if args.single_pred_prompt:
                    if args.lang == 'cn':
                        ...
                        if j == 0:
                            qs = qs + '\n' + "Let’s think step-by-step. Answer with the option's letter from the given choices directly first. Then followed by the reasons. "
                            # qs = qs + '\n' + "Answer with the option's letter from the given choices directly."
                        elif outputs is not None:
                            qs = qs + '\n' + f"The previous answer is {outputs}. Carefully review and refine your answer. Answer only with the option's letter from the given choices directly. "
                ...
                image_tensor = process_images([image], image_processor, model.config)[0].float()
